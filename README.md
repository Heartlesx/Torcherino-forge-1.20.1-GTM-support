...
## GregTech-compatible fork

This repository contains a modified Forge 1.20.1 build based on the upstream Torcherino project.
The modification detects GTCEu (GregTech CEu) machines and advances their recipes without
running additional machine ticks, so recipe progress is accelerated without multiplying EU
consumption and causing avoidable power loss.

The distributed JAR is available under `dist/`. It is built from the source in this repository.
This fork is not affiliated with or endorsed by the original Torcherino maintainers, GregTech CEu,
or the Monifactory modpack.

The original project and this modification are licensed under the GNU Lesser General Public
License, version 3 (LGPL-3.0-only). Copyright and license notices are retained in `LICENSE`.
